...
def find_quality_zone(df, trend, current_atr):
    ...
    if trend == "UP" and not fractal_lows.empty:
        ...
        if not red_candles.empty:
            ...
            has_fvg = check_fvg(df, pos, "UP")
            has_bos = check_bos_choch(df, pos, "UP")
            
            if has_fvg and has_bos:
                has_whale = check_whale_vol(df, pos, ob['vol'])
                return (ob['high'] + ob['low']) / 2, ob['low'] - (current_atr * SL_ATR_MULTIPLIER), pos, True, has_whale
        ...
        if not green_candles.empty:
            ...
            pos = df.index.get_loc(ob.name)
            has_fvg = check_fvg(df, pos, "DOWN")
            has_bos = check_bos_choch(df, pos, "DOWN")
            
            if has_fvg and has_bos:
                has_whale = check_whale_vol(df, pos, ob['vol'])
                return (ob['high'] + ob['low']) / 2, ob['high'] + (current_atr * SL_ATR_MULTIPLIER), pos, True, has_whale
...
def is_ob_fresh(df, ob_idx, sl, tp, trend):
    ...
    if trend == "UP":
        if (history['low'] <= sl).any(): return False 
    else:
        if (history['high'] >= sl).any(): return False 
    return True
